...
import { IconWrapper, TooltipWrapper, containerStyle, containerWithIconStyle, triggerStyle } from './styled';
import { Placements } from './types';

export interface IBasicTooltipProps extends Partial<Omit<TooltipTriggerProps, 'tooltip' | 'children'>> {
...
export function BasicTooltip({
  ...
  return (
    <TooltipTrigger
      ...
      delayHide={delayHide}
      {...props}
      tooltip={({ arrowRef, tooltipRef, getArrowProps, getTooltipProps, placement }): JSX.Element => (
        <div
          {...getTooltipProps({
            ref: tooltipRef,
            className: cx(containerStyle, Boolean(icon) && containerWithIconStyle, className),
          })}
        >
          {!hideArrow && (
            <div
