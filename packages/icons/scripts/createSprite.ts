...
const createSpriteForCollection = async (collectionConfig: CollectionConfig) =>
  createSprite(collectionConfig)
    .then(() => console.info(`sprite "${collectionConfig.fileName || FILENAME}" was successfully created`))
    .catch(error =>
      console.error(`error while creating sprite "${collectionConfig.fileName || FILENAME}": ${error.message}`),
    );
...
(async () => {
  for (const collection of COLLECTIONS) {
    await createSpriteForCollection(collection);
  }

  console.info('All sprites have been created.');
})();
