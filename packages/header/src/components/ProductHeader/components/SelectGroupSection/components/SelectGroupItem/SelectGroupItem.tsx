...
export const SelectGroupItem = forwardRef<HTMLDivElement, SelectGroupItemProps>(
  ({ name, selected, onClick, onMouseDown, onKeyDown, logo, actions }, ref) => {
    ...
    return (
      ...
      >
        ...
        <span className={styles.itemLabel} data-has-action-button={Boolean(showActionsButton) || undefined}>
        ...
        {showActionsButton &&
          (actions.length === 1 ? (
            ...
            <Droplist
              ...
              triggerElement={<ButtonFunction icon={<MoreSVG />} size='xs' />}
              placement='bottom-end'
            >
              {actions.map(action => (
                <Droplist.ItemSingle {...action} key={action.option} onClick={handleActionListItemClick(action)} />
              ))}
            </Droplist>
          ))}
      </div>
