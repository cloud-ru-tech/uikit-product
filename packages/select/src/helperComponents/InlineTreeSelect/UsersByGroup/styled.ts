...
export const selectClassname = css`
  &[data-filtered] {
    .mlspace .mlspace-treenode:not(.filter-node) {
      display: none;
      visibility: hidden;
    }
  }
  ...
  .mlspace-treenode-expanded:hover ~ .mlspace-treenode:not(.mlspace-treenode-expanded) {
    background: var(${COLORS_SELECT.BACKGROUND_HOVER});
  }
  .mlspace-treenode-expanded:hover ~ .mlspace-treenode-expanded ~ .mlspace-treenode {
  ...
  .mlspace .mlspace-treenode span.mlspace-switcher {
    ...
    background-image: url("data:image/svg+xml,%3Csvg width='20' height='20' viewBox='0 0 20 20' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M5.83337 11.6667L10 7.5L14.1667 11.6667H5.83337Z' fill='%23343F48'/%3E%3C/svg%3E%0A");
  ...
  .mlspace .mlspace-treenode span.mlspace-checkbox.mlspace-checkbox-indeterminate.mlspace-checkbox-disabled {
    position: relative;
    background: var(${COLORS_SELECT.DISABLED_BACKGROUND});
    border-radius: 3px;
  }
  .mlspace .mlspace-treenode span.mlspace-checkbox.mlspace-checkbox-indeterminate.mlspace-checkbox-disabled::after {
    position: absolute;
    top: 5px;
    left: 3px;
    width: 5px;
