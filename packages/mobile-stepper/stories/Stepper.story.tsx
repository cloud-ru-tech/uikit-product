...
import { MobileStepper, MobileStepperProps, useMobileStepperApi } from '../src';
...
type StoryProp = MobileStepperProps & {
  isValid: boolean;
  validationTimeout: number;
};

function ControlPanel() {
  const { goPrev, goNext, isCompleted, currentStepIndex, resetValidation } = useMobileStepperApi();
...
const Template: StoryFn<StoryProp> = ({ validationTimeout, isValid, ...args }: StoryProp) => {
  ...
  return (
    <MobileStepper {...args} onCompleteChange={() => {}} validator={validator}>
      {({
        stepper,
