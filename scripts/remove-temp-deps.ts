...
const loadYAML = <T extends Record<string, never>>(filePath: string): T => YAML.parse(readFileSync(filePath)) as T;
...
export const removeFromPnpmLock = (depsWithVersion: string[]) => {
  ...
  depsWithVersion.map(extractPackageName).forEach(pkgName => removePackageRecursively(pnpmLock, pkgName));

  saveYAML(pnpmLockPath, pnpmLock);
  console.info('Temporary dependencies removed from pnpm-lock.yaml');
};
