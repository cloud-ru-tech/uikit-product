...
const declOfNumSubFunction = (titles: string[], number: number): string => {
  ...
  return titles[
    absoluteNumber % 100 > 4 && absoluteNumber % 100 < 20
      ? 2
      : cases[absoluteNumber % 10 < 5 ? absoluteNumber % 10 : 5]
  ];
};

const declination = (rest: (number: number) => string[] | string[]) => (
  number: number,
): string =>
  declOfNumSubFunction(
    typeof rest === 'function' ? rest(number) : rest,
    number,
  );
