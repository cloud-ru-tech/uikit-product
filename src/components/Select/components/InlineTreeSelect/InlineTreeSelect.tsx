...
export interface IInlineTreeSelectProps {
  ...
  options: IOptionType[];
  onChange?: (cheched: CheckedType | undefined) => void;
...
export const InlineTreeSelect: FC<IInlineTreeSelectProps> = ({
  ...
  return (
    <>
      ...
      {open && (
        <Content>
          <InputSearch value={search} onChange={setSearh} />
      ...
      if (children) {
        searchProps.forEach(prop => {
          data = [
            ...data,
            ...children.map(
              (option: IOptionType) => option[prop as keyof IOptionType],
            ),
          ];
        });
      }
          ...
          filter={
            search
              ? (node): boolean => {
                  ...
                  return Boolean(
                    data.filter(
                      title => lCase(title).indexOf(lCase(search)) !== -1,
                    ).length,
                  );
                }
              : undefined
