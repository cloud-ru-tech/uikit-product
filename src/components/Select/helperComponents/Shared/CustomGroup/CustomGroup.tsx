...
> = props => {
  ...
  const someSelected =
    selected?.length &&
    options.some(option => selected.indexOf(option.data) !== -1);
...
export const CustomGroup = <CustomOptionType,>(
  ...
  if (!collapsedGroup) {
    return (
      data: React.ComponentProps<typeof ReactSelectComponents.Group>,
    ): JSX.Element => <ReactSelectComponents.Group {...data} />;
  }
