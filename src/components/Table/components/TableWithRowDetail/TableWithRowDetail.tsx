...
import { GroupDisabledCell } from 'components/Table/helperComponents/FrameworkComponents';
import { radioStyle } from 'components/Table/styles/tableWithRowDetail';
...
export interface ITableWithRowDetailProps {
  ...
  onSelectionChanged?(
    selectedRows?: TableBasicTypes.GridOptions['rowData'],
  ): void;
}
...
export const TableWithRowDetail: FC<ITableWithRowDetailProps> = props => {
  ...
  const handlerGroupOpened = (
    event: TableBasicTypes.RowGroupOpenedEvent,
  ): void => {
    if (event.node.expanded) {
      gridApi?.forEachNode(node => {
  ...
  return (
    <Table
      ...
      getRowClass={({ data: { disabled } }: TableBasicTypes.RowClassParams) =>
