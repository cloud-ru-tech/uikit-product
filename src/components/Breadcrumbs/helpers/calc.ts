...
export const isEllipsisActive = (element: HTMLElement): boolean =>
  getWidth(element) < element.scrollWidth;

export const getSubstr = (str: string): string =>
  str.length > CRUMB_MAX_LENGTH + 3
    ? `${str.substring(0, CRUMB_MAX_LENGTH)}...`
    : str;

export const setUniqueKey = (items: BreadcrumbItem[]): StateItem[] =>
  items.map(
    item =>
      ({ ...item, visible: true, key: item.key || nanoid() } as StateItem),
  );
