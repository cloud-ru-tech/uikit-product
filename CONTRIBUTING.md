...
- `test:cypress` - запускает cypress студию и прогоняет тесты
...
1. Запустите локальный storybook через `storybook:all` или `storybook:partial`  
2. Откройте `http://localhost:6006/` для проверки работоспособности  
3. Запустите команду `cypress open` для открытия Cypress  
4. В Cypress в списке найденных в проекте файлов с тестами выберите интересующий. Запустится выполнение всех тестов в выбранном файле.  
5. Любое изменение кода выбранного файла с тестами перезапустит их выполнение
6. Можно перезапускать тесты вручную с помощью кнопки **Run all tests** (**CMD+R**)
7. Можно запускать все/определённые тесты в файле в среде тестирования Intellij нажимая на зелёную кнопку запуска теста/группы тестов или через контекстное файла с тестами (требуется установленный плагин [Cypress Support](https://plugins.jetbrains.com/plugin/13819-cypress-support/))  